# Documentação: Centralizar Verticalmente o Conteúdo da Hero

## ⚠️ AVISO IMPORTANTE

Esta implementação modifica o posicionamento vertical do conteúdo da seção Hero para centralizá-lo dentro da seção de 100vh, sem alterar o tamanho da seção nem quebrar a responsividade.

---

## 📋 Objetivo

Centralizar verticalmente todo o conteúdo da seção Hero dentro da seção de 100vh (`h-screen`), garantindo que:
...
6. O conteúdo continue lindo e centralizado horizontalmente
...
```tsx
<section className="relative h-screen pt-12 sm:pt-16 lg:pt-20 pb-12 sm:pb-16 lg:pb-20 bg-[#452911]">
...
- Padding top: `pt-12 sm:pt-16 lg:pt-20` (48px/64px/80px)
- Padding bottom: `pb-12 sm:pb-16 lg:pb-20` (48px/64px/80px)
- Container interno: `max-w-7xl mx-auto px-4 sm:px-6 lg:px-8`
- **Problema:** O conteúdo está alinhado ao topo (padding-top), deixando muito espaço vazio no bottom
...
│  │                                 │ │ ← Espaço equilibrado
│  │ ┌───────────────────────────┐ │ │
│  │ │ H1                        │ │ │ ← Conteúdo centralizado
│  │ │ Grid Principal            │ │ │   verticalmente
│  │ │ Grid Métricas             │ │ │
│  │ └───────────────────────────┘ │ │
│  │                                 │ │ ← Espaço equilibrado
│  └───────────────────────────────┘ │
...
1. **Usar Flexbox para Centralização Vertical:**
   - Adicionar `flex flex-col justify-center` no container principal
...
2. **Manter Padding Responsivo:**
   ...
   - Remover ou reduzir paddings verticais (`pt-*` e `pb-*`) já que o flexbox vai centralizar
   - Ou manter paddings mínimos apenas para espaçamento do header fixo

3. **Preservar Estrutura:**
   - Manter toda a estrutura interna (H1, Grid Principal, Grid Métricas)
   - Manter responsividade
   - Manter alinhamento horizontal (já está centralizado)
...
**Mudanças:**
- Remover `pt-12 sm:pt-16 lg:pt-20` (padding-top)
- Remover `pb-12 sm:pb-16 lg:pb-20` (padding-bottom)
...
- [ ] 1.1. Remover `pt-12 sm:pt-16 lg:pt-20` (padding-top)
- [ ] 1.2. Remover `pb-12 sm:pb-16 lg:pb-20` (padding-bottom)
- [ ] 1.3. Adicionar `flex flex-col justify-center` para centralização vertical
- [ ] 1.4. Manter `h-screen` (altura fixa de 100vh)
- [ ] 1.5. Manter `relative` e `bg-[#452911]`
...
### Fase 3: Testes
- [ ] 3.1. Verificar que conteúdo está centralizado verticalmente
- [ ] 3.2. Verificar que não há muito espaço no topo
- [ ] 3.3. Verificar que não há muito espaço no bottom
- [ ] 3.4. Verificar que seção mantém 100vh
- [ ] 3.5. Testar em diferentes resoluções (mobile, tablet, desktop)
- [ ] 3.6. Verificar que header fixo não sobrepõe conteúdo (se necessário, adicionar padding-top mínimo)
...
**Garantias:**
- ✅ H1 mantém seu estilo e posicionamento
- ✅ Grid principal mantém sua estrutura
- ✅ Grid de métricas mantém sua estrutura
...
H1 (no topo)
  ↓
Grid Principal (no topo)
  ↓
Grid Métricas (no topo)
  ↓
padding-bottom: 48-80px
  ↓
...
Espaço equilibrado no topo ✅
  ↓
H1 (centralizado verticalmente)
  ↓
Grid Principal (centralizado verticalmente)
  ↓
Grid Métricas (centralizado verticalmente)
  ↓
...
|----------|--------------------------|----------------------|
| **Section** | `h-screen pt-12/16/20 pb-12/16/20` | `h-screen flex flex-col justify-center` |
...
| **Espaço topo** | Muito pequeno (apenas padding) | Equilibrado (flexbox) |
| **Espaço bottom** | Muito grande (muito espaço vazio) | Equilibrado (flexbox) |
...
- [ ] Não há muito espaço vazio no bottom
- [ ] Seção mantém 100vh
- [ ] Header fixo não sobrepõe conteúdo (se necessário, adicionar padding-top)

### Testes de Layout
- [ ] H1 está visível e bem posicionado
- [ ] Grid principal está visível e bem posicionado
- [ ] Grid de métricas está visível e bem posicionado
...
- 🎯 **Objetivo:** Centralizar verticalmente o conteúdo da Hero
...
5. **Verificar se header fixo não sobrepõe (se necessário, adicionar padding-top mínimo)**
6. **Validar visual em diferentes resoluções**
...
4. **Header Fixo:** Se o header fixo sobrepor o conteúdo, adicionar padding-top mínimo apenas no container interno.

5. **Responsividade:** Todas as mudanças são responsivas e funcionam em todas as resoluções.

---

**IMPORTANTE:** Esta documentação propõe uma solução simples e eficaz usando flexbox. Seguir **exatamente** como descrito.
